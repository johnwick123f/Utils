...
def run_api(response_function=None, app=None, port=2200):
    stream = Stream(
        handler=ReplyOnPause(response, can_interrupt=True),
        modality="audio",
        mode="send-receive"
    )
    stream.mount(app)
    uvicorn_thread = threading.Thread(target=run_uvicorn_in_thread, args=(app, "0.0.0.0", port))
    uvicorn_thread.daemon = True #print("To stop the server, you may need to interrupt the Kaggle kernel or restart the session.")
